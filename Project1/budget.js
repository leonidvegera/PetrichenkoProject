...
//переводим в функцию расчет дневного бюджета и вывод на екран
...
detectDayBudget();  //чтобы расчитать дневной бюджет и вывести его запускаем функцию

//переводим в функцию расчет уровня достатка
...
detectLevel();    //чтобы расчитать уровень достатка запускаем функцию
...
//создаем функцию для расчета необязательных расходов
function chooseOptExpenses() {
  for (let i = 1; i < 4; i++) {               //итерацию начинаем с 1, вопрос задаем 3 раза
    let askOptionExpens = +prompt('Статья необязательных расходов?',);   //задаём вопрос для получения значения
    appData.optionalExpenses[i] = askOptionExpens;       //с каждой итерацией добавляем в объект изменённое свойство optionalExpenses, с данными полученными от пользователя в ключ - номер итерации начиная с 1, и ответ на вопрос в значение
  }
}

chooseOptExpenses();    //чтобы начать задавать вопросы о необязательных расходах запускаем функцию
